...
class Video_model extends CI_Model
{
	...
	public function get_by_category_id($kategori)
	{
		$this->db->where('video.category_video_id',$kategori);
		$this->db->order_by('date_video', 'DESC');
		$this->db->where_not_in('id_video', $this->id_video);
		$this->db->limit(3);
		$query = $this->db->get('video');
		return $query->result();
	}
	...
	public function get_for_page($limit,$offset)
	{
		...
		if ($this->search!=""){
			$this->db->group_start();
			$this->db->like('judul',$this->search);
			$this->db->or_like('content',$this->search);
			$this->db->group_end();
		}
