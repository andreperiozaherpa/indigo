...
class Surat_masuk_model extends CI_Model
{
	...
	{
		$detail = $this->get_detail_verifikasi($id_surat_masuk_verifikasi);
		if ($detail) {
	...
	public function add_by_surat_keluar($id_surat_keluar, $testing = "")
	{
		...
		foreach ($get as $row) {
			...
			if ($id_skpd_penerima == "231") {
				...
				$kepala_setwan = $this->master_pegawai_model->get_by_id(10782);
				$data = array(
	...
	//    Edited by Dinar
	public function get_all_where($search, $year, $skpd, $page, $arrIinbox)
	{
		$this->db->where('id_skpd_penerima', $skpd);
		$this->db->where("YEAR(tanggal_surat)", $year);
		$this->db->where('jenis_surat', 'eksternal');

		$this->db->where_not_in('id_surat_masuk', $arrIinbox);
		...
		$this->db->order_by('id_surat_masuk', 'DESC');
		$this->db->select('id_surat_masuk, perihal, nomer_surat, hash_id');
		$result = $this->db->get('surat_masuk')->result();

		if ($result > 0) {
			foreach ($result as $res) {
				$res->id = $res->id_surat_masuk;
	...
	public function get_single_row($id_surat_masuk)
	{
		$this->db->select('id_skpd_penerima');
