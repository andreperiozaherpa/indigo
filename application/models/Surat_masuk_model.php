...
class Surat_masuk_model extends CI_Model
{
	...
	public function get_urutan_surat($id_skpd)
	{
		...
		$this->db->where('no_urut is not null');
		$get = $this->db->get('surat_masuk');
		if ($get->num_rows() == 0) {
			$nomor_urut = 1;
		} else {
			$fetch = $get->row();
			$nomor_urut = $fetch->no_urut + 1;
		}
		return $nomor_urut;
	}
	...
	public function get_page_eksternal_disposisi($mulai, $hal, $filter = '', $summary_field = '', $summary_value = '')
	{
		...
		$this->db->join('pegawai as pegawai_penerima', 'disposisi_surat_masuk.id_pegawai = pegawai_penerima.id_pegawai', 'left');
	...
	public function get_page_verifikasi($mulai, $hal, $filter = '')
	{
		// $this->db->offsett(0,6);
		if ($filter != '') {
			foreach ($filter as $key => $value) {
		...
		// $this->db->where('surat_masuk_verifikasi.jenis_surat', "eksternal");
		// $this->filter_user();
		$this->db->join('ref_skpd', 'surat_masuk_verifikasi.id_skpd_penerima = ref_skpd.id_skpd');
		$this->db->join('pegawai', 'surat_masuk_verifikasi.id_pegawai_penerima = pegawai.id_pegawai');
		// $this->db->order_by('surat_masuk_verifikasi.status_surat', 'ASC');
		$this->db->select('surat_masuk_verifikasi.*,ref_skpd.nama_skpd, pegawai.nama_lengkap as nama_penerima');
		$this->db->where('id_verifikator', $this->session->userdata('id_pegawai'));
		$this->db->order_by('surat_masuk_verifikasi.id_surat_masuk_verifikasi', 'DESC');

		$query = $this->db->get('surat_masuk_verifikasi');
		return $query->result();
	}
	...
	public function get_surat_masuk($mulai = null, $hal = null, $filter = '', $summary_field = '', $summary_value = '', $android = false)
	{
		...
		$url_eoffice = base_url();
		$url_eoffice_desa = "https://e-officedesa.sumedangkab.go.id";

		$path_surat = '/data/surat_internal/surat_masuk/';

		$select_url = "
			CASE 
				WHEN surat_desa = 1 THEN CONCAT('" . $url_eoffice_desa . "','/data/surat_',jenis_surat,'/surat_masuk/',file_surat)
				ELSE CONCAT('" . $url_eoffice . "','/data/surat_',jenis_surat,'/surat_masuk/',file_surat)
			END as 'url_surat'
		";

		$select = "surat_masuk.*,ref_skpd.*,IF(surat_masuk.surat_desa=1,'user_default.png', pegawai.foto_pegawai) as user_picture ,$select_url";
	...
	public function get_detail_verifikasi($id_surat_masuk_verifikasi)
	{
		$this->db->join('pegawai', 'pegawai.id_pegawai = surat_masuk_verifikasi.id_pegawai_teruskan', 'left');
		return $this->db->get_where('surat_masuk_verifikasi', ['id_surat_masuk_verifikasi' => $id_surat_masuk_verifikasi])->row();
	}
	public function get_detail_verifikasi_raw($id_surat_masuk_verifikasi)
	{
		return $this->db->get_where('surat_masuk_verifikasi', ['id_surat_masuk_verifikasi' => $id_surat_masuk_verifikasi])->row();
	}

	public function terima_verifikasi($id_surat_masuk_verifikasi, $id_pegawai)
	{
		$detail = $this->get_detail_verifikasi_raw($id_surat_masuk_verifikasi);
		if ($detail) {
			unset($detail->id_surat_masuk_verifikasi);
			unset($detail->id_verifikator);
			unset($detail->status_verifikasi);
			unset($detail->id_pegawai_teruskan);
			unset($detail->id_pegawai_teruskan);
			unset($detail->tgl_verifikasi);
			$data_insert = (array) $detail;
			$this->load->model('pegawai_dewan_model');
			$detail_pegawai = $this->pegawai_dewan_model->get_pegawai_by_id($id_pegawai);
			$data_insert['id_pegawai_penerima'] = $id_pegawai;
			$data_insert['id_skpd_penerima'] = $detail_pegawai->id_skpd;
			$data_insert['id_unitkerja_penerima'] = $detail_pegawai->id_unit_kerja;
			// $in = $this->db->insert('surat_masuk', $data_insert);
			$in = $this->pegawai_dewan_model->insert_surat_masuk($data_insert);
			if ($in) {
				$insert_id = $in;
				$this->db->update('surat_masuk_verifikasi', ['status_verifikasi' => 'sudah_diverifikasi', 'id_pegawai_teruskan' => $id_pegawai, 'id_surat_masuk' => $insert_id, 'tgl_verifikasi' => date('Y-m-d H:i:s')], ['id_surat_masuk_verifikasi' => $id_surat_masuk_verifikasi]);
				// $id_user_penerima = $this->getUserIdPegawai($id_pegawai);
				$id_user_penerima = $this->pegawai_dewan_model->get_pegawai_user($id_pegawai);
	...
	public function get_detail_disposisi_keluar_by_id($id)
	{
		...
		$select_url = "
			CASE 
				WHEN surat_masuk.surat_desa = 1 THEN CONCAT('" . $url_eoffice_desa . "','/data/surat_',surat_masuk.jenis_surat,'/surat_masuk/',surat_masuk.file_surat)
				ELSE CONCAT('" . $url_eoffice . "','/data/surat_',surat_masuk.jenis_surat,'/surat_masuk/',surat_masuk.file_surat)
			END as 'url_surat'
		";
	...
	public function add_by_surat_keluar($id_surat_keluar, $testing = "")
	{
		...
		$this->db->select("*, surat_keluar_penerima.id_skpd as id_skpd_eksternal, pegawai_input.nama_lengkap as nama_lengkap_input, pegawai_penerima.nama_lengkap as nama_lengkap_penerima, pegawai_penerima.id_skpd as id_skpd_penerima, pegawai_penerima.id_unit_kerja as id_unit_kerja_penerima, pegawai_penerima.id_pegawai as id_pegawai_penerima, user.app_token, surat_keluar_penerima.tujuan as tujuan_jabar");
		...
		foreach ($get as $k => $row) {
			...
			if ($id_skpd_penerima == "231") {
				...
				$data = array(
					...
					'hash_id' => $row->hash_id,
					'id_verifikator' => $kepala_setwan->id_pegawai
				);
				$this->db->insert('surat_masuk_verifikasi', $data);
				$insert_id = $this->db->insert_id();
				$id_user_penerima = $this->getUserIdPegawai($data['id_verifikator']);
				$after_insert_data[] = array('id_user' => $id_user_penerima, 'id_surat_masuk_verifikasi' => $insert_id);
			} else {
				if ($row->jenis_surat == "internal" or $row->jenis_penerima == "skpd") {
					...
					// }
				} else if ($row->jenis_penerima == 'jabar') {
					// date YmdHis with milisecond
					$date = date('YmdHis');
					$milliseconds = round(microtime(true) * 1000);

					$milliseconds = substr($milliseconds, -3);
					$tid = "3211" . $date . $milliseconds . $k;

					$update_tid = $this->db->update('surat_keluar_penerima', ['tid' => $tid], ['id_surat_keluar_penerima' => $row->id_surat_keluar_penerima]);

					$pegawai_ttd = $this->db->get_where('pegawai', array('id_pegawai' => $row->id_pegawai_ttd))->row();

					$data_send_surat = [
						'unit_pencipta' => md5('mKashj89@1' . $row->id_skpd_pengirim),
						'tid' => $row->tid,
						'nomor_naskah' => $row->nomer_surat,
						'kode_klasifikasi' => $row->jabar_kode_klasifikasi,
						'tanggal' => date('Y-m-d H:i:s', strtotime($row->tgl_surat)),
						'tujuan' => '["' . $row->tujuan_jabar . '"]',
						'hal' => $row->perihal,
						'jenis_naskah' => $row->jabar_jenis_naskah,
						'isi' => '',
						'sifat_naskah' => $row->jabar_sifat_naskah,
						'jenis_lampiran' => $row->jabar_jenis_lampiran,
						'jumlah_lampiran' => $row->lampiran,
						'tembusan' => '[]',
						'penandatangan' => $pegawai_ttd->nama_lengkap,
						'tempat_penandatangan' => 'Sumedang',
						'tanggal_persetujuan' => date('Y-m-d H:i:s', strtotime($row->tgl_ttd)),
						'nip_pejabat' => $pegawai_ttd->nip,
						'nama_pejabat' => $pegawai_ttd->nama_lengkap,
						'jabatan' => $pegawai_ttd->jabatan,
						'file' => curl_file_create('./data/surat_eksternal/ttd/' . $row->file_ttd, 'application/pdf', 'file_surat.pdf'),
						'kode_wilayah' => 3211,
					];

					// print_r($data_send_surat);
					// die;

					$get_eo = $this->db->get_where('surat_penerima_jabar', ['kode_wilayah' => $row->kode_wilayah_jabar])->row();
					if ($get_eo) {
						$send_to_jabar = $this->splpjabar->sendSurat($get_eo->path_endpoint, $data_send_surat);
						// print_r($send_to_jabar);
					}
