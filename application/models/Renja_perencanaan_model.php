...
class Renja_perencanaan_model extends CI_Model
{
	public function name($jenis)
	{
		...
		$tSasaranInisiatif = $name . '_inisiatif';
		$cSasaranInisiatif = 'id_' . $tSasaranInisiatif;
		...
		return array(
			'cSasaran' => $cSasaran, 'tSasaran' => $tSasaran, 'tIku' => $tIku, 'cIku' => $cIku, 'cUnitKerja' => $cUnitKerja, 'name' => $name, 'nSasaran' => $nSasaran, 'tIkuRenja' => $tIkuRenja, 'cIkuRenja' => $cIkuRenja, 'taIkuRenja' => $taIkuRenja, 'aIkuRenja' => $aIkuRenja, 'rIkuRenja' => $rIkuRenja, 'tRenaksi' => $tRenaksi,
			'cRenaksi' => $cRenaksi, 'tTaRenaksi' => $tTaRenaksi, 'cTaRenaksi' => $cTaRenaksi, 'tSasaranInisiatif' => $tSasaranInisiatif, 'cSasaranInisiatif' => $cSasaranInisiatif
		);
	}
	...
	public function get_iku_by_penanggungjawab($key, $id_unit_kerja)
	{
		$name = $this->name($key);
		if ($key !== 'ss') {
			$this->db->where('FIND_IN_SET(' . $id_unit_kerja . ', id_unit_kerja)');
			$query = $this->db->get($name['tSasaran'])->result();
			$iku = array();
			foreach ($query as $q) {
				$cSasaran = $name['cSasaran'];
				$id_renstra = $q->$cSasaran;
				$this->db->where($name['cSasaran'], $id_renstra);
				$this->db->join('ref_satuan', 'ref_satuan.id_satuan = ' . $name['tIku'] . '.id_satuan');
				$g_iku = $this->db->get($name['tIku'])->result();
				foreach ($g_iku as $g) {
					$iku[] = $g;
				}
			}
	...
	public function get_sasaran_inisiatif($jenis, $id_unit_kerja, $tahun)
	{	$name = $this->name($jenis);
		$this->db->where('tahun_renja', $tahun);
		$this->db->where('inisiatif', 1);
		$this->db->where('jenis_renja', 'unit_kerja');
		$this->db->where($name['tIkuRenja'] . '.id_unit_kerja', $id_unit_kerja);
		$this->db->group_by($name['tIkuRenja'] . '.' . $name['cSasaranInisiatif']);
		$this->db->join($name['tSasaranInisiatif'], $name['tSasaranInisiatif'] . '.' . $name['cSasaranInisiatif'] . ' = ' . $name['tIkuRenja'] . '.' . $name['cSasaranInisiatif']);
	...
	public function get_iku_inisiatif($jenis, $id_sasaran, $tahun, $id_unit_kerja)
	{
		$name = $this->name($jenis);
		$this->db->where('tahun_renja', $tahun);
		$this->db->where($name['tIkuRenja'] . '.' . $name['cSasaranInisiatif'], $id_sasaran);
		$this->db->where('inisiatif', 1);
		$this->db->where('jenis_renja', 'unit_kerja');
		$this->db->where($name['tIkuRenja'] . '.id_unit_kerja', $id_unit_kerja);
		$this->db->join($name['tSasaranInisiatif'], $name['tSasaranInisiatif'] . '.' . $name['cSasaranInisiatif'] . ' = ' . $name['tIkuRenja'] . '.' . $name['cSasaranInisiatif']);
		$this->db->join('ref_satuan', 'ref_satuan.id_satuan = ' . $name['tIkuRenja'] . '.id_satuan_inisiatif');
		$this->db->join('ref_unit_kerja','ref_unit_kerja.id_unit_kerja = '.$name['tIkuRenja'].'.id_unit_kerja');
		$query = $this->db->get($name['tIkuRenja'])->result();
		foreach ($query as $n => $q) {
			$query[$n]->polorarisasi = $q->polorarisasi_inisiatif;
			$tIku = $name['tIku'];
			$query[$n]->$tIku = $q->nama_indikator_inisiatif;
		}
		return $query;
	}
	...
	public function get_sasaran_skpd_inisiatif($jenis, $id_skpd, $tahun)
	{
		$name = $this->name($jenis);
		$this->db->where('tahun_renja', $tahun);
		$this->db->where('jenis_renja', 'skpd');
	...
	public function get_iku_inisiatif_skpd($jenis, $id_sasaran, $tahun, $id_skpd)
	{
		$name = $this->name($jenis);
		$this->db->where('tahun_renja', $tahun);
		$this->db->where($name['tIkuRenja'] . '.' . $name['cSasaranInisiatif'], $id_sasaran);
	...
	public function get_iku_renja_by_id($jenis, $id_iku, $is_inisiatif = 0)
	{
		$name = $this->name($jenis);
		...
		if($is_inisiatif==1){
			...
			$tIku = $name['tIku'];
			$q->$tSasaran = $q->nama_sasaran;
			$q->$tIku = $q->nama_indikator_inisiatif;
			$q->polorarisasi = $q->polorarisasi_inisiatif;
		}
		return $q;
	}
	...
	public function get_target_renaksi_by_id($jenis, $id_target_renaksi,$is_inisiatif=0)
	{
		$name = $this->name($jenis);
		...
		$q = $this->db->get($name['tTaRenaksi'])->row();
		if($is_inisiatif==1){
			$tSasaran = $name['tSasaran'];
			$tIku = $name['tIku'];
			$q->$tIku = $q->nama_indikator_inisiatif;
	...
	public function insert_sasaran_inisiatif($jenis, $data)
	{
		$name = $this->name($jenis);
		$this->db->insert($name['name'] . '_inisiatif', $data);
		return $this->db->insert_id();
	}

	public function insert_indikator_inisiatif($jenis, $data)
	{
	}
}
