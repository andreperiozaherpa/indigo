...
class Ref_kelas_jabatan_model extends CI_Model
{
	...
	{
		$this->db->order_by('simanja_ref_kelas_jabatan.kelas');
		$query = $this->db->get('simanja_ref_kelas_jabatan');
		return $query->result();
	}

	public function get_by_id($id){
		return $this->db->get_where('simanja_ref_kelas_jabatan',array('id'=>$id))->row();
	}

	public function get_by_id_skpd($id_skpd){
		return $this->db->get_where('simanja_ref_kelas_jabatan',array('id_skpd'=>$id_skpd))->result();
	}

	public function insert_ref($data)
	{
		$query = $this->db->insert('simanja_ref_kelas_jabatan',$data);
	...
	{
        $this->db->where('id', $id);
        $query = $this->db->update('simanja_ref_kelas_jabatan',$data);
	}
	
	public function delete_ref($id = NULL)
	{
		$this->db->where('id',$id);
		$query = $this->db->delete('simanja_ref_kelas_jabatan');	
		// redirect('ref_kelas_jabatan');
	}

	public function cek($data){
		$q = $this->db->get_where('simanja_ref_kelas_jabatan',array('id_skpd'=>$data['id_skpd'],'id_unit_kerja'=>$data['id_unit_kerja'],'nama_jabatan'=>$data['nama_jabatan'],'jenis_jabatan'=>$data['jenis_jabatan']))->row();
		return $q;
	}
	...
	public function get($param)
	{
		...
		$rs = $this->db->get("simanja_ref_kelas_jabatan jabatan");
		return $rs;
	}
	...
	public function insert_excel($data){
		$insert = $this->db->insert_batch('simanja_ref_kelas_jabatan', $data);
		if($insert){
			return true;
		}
	}

	public function get_by_nilai($nilai){
		$this->db->where('nilai_minimal <=', $nilai);
		$this->db->where('nilai_maksimal >=', $nilai);
		return $this->db->get('simanja_ref_kelas_jabatan')->row();
	}
}
?>