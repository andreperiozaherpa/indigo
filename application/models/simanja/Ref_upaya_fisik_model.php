...
class Ref_upaya_fisik_model extends CI_Model
{
	...
	{
		$this->db->order_by('simanja_ref_upaya_fisik.kode');
		$query = $this->db->get('simanja_ref_upaya_fisik');
		return $query->result();
	}

	public function get_by_id($id){
		return $this->db->get_where('simanja_ref_upaya_fisik',array('id'=>$id))->row();
	}

	public function get_by_id_skpd($id_skpd){
		return $this->db->get_where('simanja_ref_upaya_fisik',array('id_skpd'=>$id_skpd))->result();
	}

	public function insert_ref($data)
	{
		$query = $this->db->insert('simanja_ref_upaya_fisik',$data);
	...
	{
        $this->db->where('id', $id);
        $query = $this->db->update('simanja_ref_upaya_fisik',$data);
	}
	
	public function delete_ref($id = NULL)
	{
		$this->db->where('id',$id);
		$query = $this->db->delete('simanja_ref_upaya_fisik');	
		// redirect('ref_upaya_fisik');
	}

	public function cek($data){
		$q = $this->db->get_where('simanja_ref_upaya_fisik',array('id_skpd'=>$data['id_skpd'],'id_unit_kerja'=>$data['id_unit_kerja'],'nama_jabatan'=>$data['nama_jabatan'],'jenis_jabatan'=>$data['jenis_jabatan']))->row();
		return $q;
	}
	...
	public function get($param)
	{
		...
		$rs = $this->db->get("simanja_ref_upaya_fisik jabatan");
		return $rs;
	}
	...
	public function insert_excel($data){
		$insert = $this->db->insert_batch('simanja_ref_upaya_fisik', $data);
		if($insert){
			return true;
		}
	}
