...
class Lembar_kerja_evaluasi_model extends CI_Model
{
    ...
    public function get_pokja($id_skpd, $jenis_lke)
    {
        ...
        WHERE  a.id_skpd="'.$id_skpd.'" AND c.jenis_lke="'.$jenis_lke.'"
        ORDER BY id DESC');
        return $get;
    }

    public function get_pokja_filter($tahun, $id_skpd, $jenis_lke)
    {
        $get = $this->db->query('select id,  a.id_lke_indikator as id_lke_indikator, 
        a.id_pegawai_ketua, c.nama_indikator as nama_indikator, c.tahun as tahun, 
        c.jenis_lke as jenis, nama_pokja, d.nama_indikator as nama_induk, nama_lengkap 
        from lke_pokja a  
                ...
                JOIN lke_indikator c ON a.id_lke_indikator=c.id_lke_indikator
                JOIN lke_indikator d ON c.id_induk=d.id_lke_indikator
        WHERE c.tahun=' . $tahun . ' AND a.id_skpd="'.$id_skpd.'" AND c.jenis_lke="'.$jenis_lke.'"
        ORDER BY id DESC');
        return $get;
    }

    public function get_lke_indikator($jenis_lke)
    {
        $get = $this->db->query('select a.id_lke_indikator as id_indikator, a.nama_indikator as nama_indikator, a.jenis_lke as jenis_lke, a.tahun as tahun,
         ...
         WHERE a.level = 3 AND a.jenis_lke="'.$jenis_lke.'"');
        return $get;
    }

    public function get_lke_indikator_filter($tahun, $jenis_lke)
    {
        $get = $this->db->query('select a.id_lke_indikator as id_indikator, a.nama_indikator as nama_indikator, a.jenis_lke as jenis_lke, a.tahun as tahun,
        b.nama_indikator as nama_induk
        FROM lke_indikator a  
        JOIN lke_indikator b ON a.id_induk=b.id_lke_indikator
        WHERE a.level = 3 AND a.tahun=' . $tahun.' AND a.jenis_lke="'.$jenis_lke.'"');
        return $get;
    }

    public function get_pegawai($id_skpd)
    {
        $get = $this->db->query('select * from pegawai where id_skpd="'.$id_skpd.'"');
        return $get;
    }
    ...
    public function skpd_ref_by_id($id_skpd)
    {
        $get = $this->db->query('select * from ref_skpd where id_skpd="'.$id_skpd.'"');
        return $get;
    }
    ...
    public function get_setting_waktu_verif($jenis_pengisi, $tahun, $id_skpd, $jenis_lke)
    {
        $this->db->where('jenis_pengisi', $jenis_pengisi);
        $this->db->where('tahun', $tahun);
