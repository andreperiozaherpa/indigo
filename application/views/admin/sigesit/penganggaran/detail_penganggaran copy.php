                            ...
                            <div class="panel-wrapper collapse in">
                                <div class="panel-body">
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody id="row-data">
                                            <tr>
                                                ...
                                                <td><?= ($detail->realisasi_anggaran) ? number_format($detail->realisasi_anggaran) : '-' ;?>
                                                </td>
                                                <td> <a href="#" data-toggle="modal" data-target="#updateRealisasi"
                                                        data-whatever="@mdo"><button
                                                            class="btn btn-outline btn-success">
                                                            Trouble (Opps)</button></a></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    ...
                                    <form id="form-realisasi">
                                        <table class="table">
                                            ...
                                            <tbody id="row_realisasi_anggaran">
                                                ...
                                                {
                                                    $action = '<button onclick="hapus_realisasi_anggaran('.$key.')" class="btn btn-secondary"><i class="fa fa-trash"></i></button>';

                                                    $id_realisasi_anggaran = !empty($row->id_realisasi_anggaran) ? $row->id_realisasi_anggaran : 0;

                                                    echo '
                                                    <tr id="realisasi_anggaran_'.$key.'">
                                                        <td><input value="'.$row->nama_kegiatan.'" id="nama_kegiatan_'.$key.'" name="realisasi_anggaran[nama_kegiatan][]" type="text" class="form-control" placeholder=""></td>
                                                        <td><input value="'.$row->jumlah.'" onkeyup="kalkulasi('.$key.')" id="jumlah_'.$key.'" name="realisasi_anggaran[jumlah][]" type="number" class="form-control" placeholder=""></td>
                                                        <td><input value="'.number_format($row->harga).'" onkeyup="kalkulasi('.$key.')" id="harga_'.$key.'" name="realisasi_anggaran[harga][]" type="text" class="form-control" placeholder=""></td>
                                                        <td><input value="'.$row->satuan.'" id="satuan_'.$key.'" name="realisasi_anggaran[satuan][]" type="text" class="form-control" placeholder=""></td>
                                                        <td><input value="'.number_format($row->total).'" id="total_'.$key.'" type="text" class="form-control" placeholder="" readonly></td>
                                                        <td>
                                                        '.$action.'
                                                        <input value="'.$id_realisasi_anggaran.'" id="id_realisasi_anggaran_'.$key.'" name="realisasi_anggaran[id_realisasi_anggaran][]" type="hidden">
                                                        </td>
                                                    </tr>
                                                    ';
                                                }
                                    ...
                                    <a href="javascript:void(0)" class="btn btn-primary btn-outline"
                                        onclick="tambah_realisasi_anggaran()"><i class="fa fa-plus"></i> Tambah
                                        Data</a>
...
<div class="modal fade" id="updateRealisasi" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1">
                ...
                <form id="form-data">
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="id_ref_program" name="id_ref_program"
                            ...
                            foreach($dt_program as $row){
                                $selected = ($dt_realisasi && $dt_realisasi->id_ref_program == $row->id_program) ? "selected" : "";
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="id_ref_kegiatan" name="id_ref_kegiatan"
                            ...
                            foreach($dt_kegiatan as $row){
                                $selected = ($dt_realisasi && $dt_realisasi->id_ref_kegiatan == $row->id_kegiatan) ? "selected" : "";
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="id_ref_sub_kegiatan" name="id_ref_sub_kegiatan">
                            ...
                            foreach($dt_sub_kegiatan as $row){
                                $selected = ($dt_realisasi && $dt_realisasi->id_ref_sub_kegiatan == $row->id_sub_kegiatan) ? "selected" : "";
                    ...
                    <div class="form-group" style="">
                        ...
                        <input type="text" onkeyup="currency(this)"
                            value="<?= ($dt_realisasi)? number_format($dt_realisasi->total_realisasi_anggaran) : '' ;?>"
                            class="form-control" name="total_realisasi_anggaran" id="total_realisasi_anggaran"
                            placeholder="Masukan Realisasi Anggaran">
                        <div class="text-danger error" id="err_total_realisasi_anggaran"></div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group" style="">
                                <label for="message-text" class="control-label">Target</label>
                                <input type="number" value="<?=$dt_realisasi->target_realisasi;?>" class="form-control"
                                    name="target_realisasi" id="target_realisasi" placeholder="Target">
                                <div class="text-danger error" id="err_target_realisasi"></div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group" style="">
                                <label for="message-text" class="control-label">Satuan</label>
                                <input type="text" value="<?=$dt_realisasi->satuan_realisasi;?>" class="form-control"
                                    name="satuan_realisasi" id="satuan_realisasi" placeholder="Target">
                                <div class="text-danger error" id="err_satuan_realisasi"></div>
                            </div>
                        </div>
                    </div>
...
<script>
    var i = '<?= ($dt_realisasi_anggaran) ? count($dt_realisasi_anggaran) : 0 ;?>';
    ...
    function save_realisasi()
    {
      ...
      formdata.append("id_kegiatan","<?=$detail->id_kegiatan;?>");
      $.ajax({
         url        : "<?=base_url()?>sigesit/penganggaran/save_realisasi",
         type       : 'post',
         dataType   : 'json',
         data       : formdata,
         processData:false,
    ...
    function tambah_realisasi_anggaran()
    {

        var action = '<button onclick="hapus_realisasi_anggaran('+i+')" class="btn btn-secondary"><i class="fa fa-trash"></i></button>';
        
        
        var row = '<tr id="realisasi_anggaran_'+i+'">'
            +'<td><input id="nama_kegiatan_'+i+'" name="realisasi_anggaran[nama_kegiatan][]" type="text" class="form-control" placeholder=""></td>'
            +'<td><input onkeyup="kalkulasi('+i+')" id="jumlah_'+i+'" name="realisasi_anggaran[jumlah][]" type="number" class="form-control" placeholder=""></td>'
            +'<td><input onkeyup="kalkulasi('+i+')" id="harga_'+i+'" name="realisasi_anggaran[harga][]" type="text" class="form-control" placeholder=""></td>'
            +'<td><input id="satuan_'+i+'" name="realisasi_anggaran[satuan][]" type="text" class="form-control" placeholder=""></td>'
            +'<td><input id="total_'+i+'" type="text" class="form-control" placeholder="" readonly></td>'
            +'<td>'
            +action
            +'<input id="total_'+i+'" name="realisasi_anggaran[id_realisasi_anggaran][]" value="0" type="hidden">'
            +'</td>'
            +'</tr>';

        $("#row_realisasi_anggaran").append(row);

        i++;

    }

    function hapus_realisasi_anggaran(x)
    {
        $("#realisasi_anggaran_"+x).remove();
    }

    setTimeout(() => {
        tambah_realisasi_anggaran();
    }, 100);
