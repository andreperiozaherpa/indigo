...
<div class="row bg-title">
  ...
  <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
    <h4 class="page-title">Detail Faktor Evaluasi - <?=$detail->nama?></h4>
  </div>
  <!-- /.page title -->
  <!-- .breadcrumb -->
  <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
    ...
    <ol class="breadcrumb">
      ...
      <li><a href="<?=base_url('simanja/ref_faktor_evaluasi')?>">Ref. Faktor Evaluasi</a></li>
    ...
    <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="panel panel-default">
          <div class="panel-heading">Detail Faktor Evaluasi 
          ...
          <div class="panel-wrapper collapse in">
            ...
            <h4>Faktor <?=$detail->number.' : '.$detail->nama?></h4>
            <p>
              <?=$detail->uraian?>
            </p>
            <br>
                ...
                <tbody>
                  <?php
                   ...
                   ?>
                    <tr>
                      <td><?= $no ?></td>
                      <td width="100px"><?= $i->level ?></td>
                      <td><?= $i->kriteria ?></td>
                      <td width="100px"><?= $i->nilai ?></td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="formRef">
          ...
          <input type="hidden" name="id_faktor_evaluasi" value="<?=$detail->id?>">
          <div class="form-group">
            <label for="message-text" class="control-label">Level</label>
            <input type="text" name="level" class="form-control" placeholder="Masukkan Level Faktor Evaluasi">
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">Kriteria</label>
            <textarea class="textarea_editor form-control" rows="10" name="kriteria"></textarea>
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">Nilai</label>
            <input type="number" name="nilai" class="form-control" placeholder="Masukkan Nilai Faktor Evaluasi">
          </div>
      </div>
...
<script type="text/javascript">
  ...
  function editRef(id) {
    ...
    $('.help-block').empty();
    $.ajax({
      url: "<?= base_url() . 'simanja/ref_faktor_evaluasi/fetch_ref_item/' ?>" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        $('[name="id_ref"]').val(data.id);
        $('[name="level"]').val(data.level);
        $('[name="kriteria"]').data("wysihtml5").editor.setValue(data.kriteria);
  ...
  function simpanRef() {
    ...
    if (save_method == 'add') {
      url = "<?= base_url() . 'simanja/ref_faktor_evaluasi/p_add_ref_item' ?>";
    } else {
      url = "<?= base_url() . 'simanja/ref_faktor_evaluasi/p_update_ref_item' ?>";
    }
  ...
  function deleteRef(id) {
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            url: "<?= base_url() . 'simanja/ref_faktor_evaluasi/delete_ref_item/' ?>" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
              $('#modalReferensi').modal('hide');
