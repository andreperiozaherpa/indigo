                  ...
                  <?php
                   ...
                   ?>
                    <tr>
                      <td><?= $no ?></td>
                      <td><?= $j->nama ?></td>
                      <td><?= $j->kualifikasi_pendidikan ?></td>
                      <td><?= $j->tugas ?></td>
                      <td><?= $j->pengalaman_kerja ?></td>
                      <td><?= $j->diklat_teknis ?></td>
                      <td><?= $j->jenis ?></td>

                      <td style="width:150px">
                        <a href="javascript:void(0)" onclick="editRef(<?= $j->id ?>)" class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i> Edit</a>
                        <a href="javascript:void(0)" onclick="deleteRef(<?= $j->id ?>)" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Delete</a>
                      </td>
                    </tr>
                  <?php $no++;
                   } ?>
      ...
      <div class="modal-body">
        <form id="formRef">
          ...
          <div class="form-group">
            ...
            <input type="text" name="nama" class="form-control" placeholder=" Masukkan Jabatan">
          </div>

          <div class="form-group">
            <label for="message-text" class="control-label">Kualifikasi Pendidikan</label>
            <textarea name="kualifikasi_pendidikan" class="textarea_editor form-control" rows="10"></textarea>
          </div>

          <div class="form-group">
            <label for="message-text" class="control-label">Tugas Jabatan</label>
            <textarea name="tugas" class="textarea_editor form-control" rows="10"></textarea>
          </div>

          <div class="form-group">
            <label for="message-text" class="control-label">Pengalaman Kerja</label>
            <textarea name="pengalaman_kerja" class="textarea_editor form-control" rows="10"></textarea>
          </div>

          <div class="form-group">
            <label for="message-text" class="control-label">Diklat Teknis</label>
            <textarea name="diklat_teknis" class="textarea_editor form-control" rows="10"></textarea>
          </div>
...
<script type="text/javascript">
  ...
  function editRef(id) {
    ...
    $('.help-block').empty();
    $.ajax({
      url: "<?= base_url() . 'simanja/ref_jabatan/fetch_ref/' ?>" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        $('[name="id_ref"]').val(data.id);
        $('[name="nama"]').val(data.nama);
        $('[name="tugas"]').data("wysihtml5").editor.setValue(data.tugas);
        $('[name="kualifikasi_pendidikan"]').data("wysihtml5").editor.setValue(data.kualifikasi_pendidikan);
        $('[name="pengalaman_kerja"]').data("wysihtml5").editor.setValue(data.pengalaman_kerja);
        $('[name="diklat_teknis"]').data("wysihtml5").editor.setValue(data.diklat_teknis);
  ...
  function simpanRef() {
    ...
    if (save_method == 'add') {
      url = "<?= base_url() . 'simanja/ref_jabatan/p_add_ref' ?>";
    } else {
      url = "<?= base_url() . 'simanja/ref_jabatan/p_update_ref' ?>";
    }
  ...
  function deleteRef(id) {
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            url: "<?= base_url() . 'simanja/ref_jabatan/delete_ref/' ?>" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
              $('#modalReferensi').modal('hide');
