...
<div class="row bg-title">
  ...
  <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
    <h4 class="page-title">Referensi Kelas Jabatan</h4>
  </div>
  <!-- /.page title -->
  <!-- .breadcrumb -->
  <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Ref. Kelas Jabatan</li>
    </ol>
  </div>
  <!-- /.breadcrumb -->
</div>
    ...
    <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="panel panel-default">
          <div class="panel-heading">Kelas Jabatan 
                  ...
                  <?php
                   ...
                   ?>
                    <tr>
                      <td><?= $no ?></td>
                      <td><?= $i->kelas?></td>
                      <td><?= $i->nilai_minimal ?></td>
                      <td><?= $i->nilai_maksimal ?></td>
      ...
      <div class="modal-body">
        <form id="formRef">
          ...
          <div class="form-group">
            <label for="message-text" class="control-label">Kelas Jabatan</label>
            <input type="text" name="kelas" class="form-control" placeholder="" >
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">Nilai Minimal</label>
            <input type="text" name="nilai_minimal" class="form-control" placeholder="" >
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">Nilai Maksimal</label>
            <input type="text" name="nilai_maksimal" class="form-control" placeholder="" >
          </div>
      </div>
...
<script type="text/javascript">
  ...
  function editRef(id) {
    ...
    $('.help-block').empty();
    $.ajax({
      url: "<?= base_url() . 'simanja/ref_kelas_jabatan/fetch_ref/' ?>" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        $('[name="id_ref"]').val(data.id);
        $('[name="kelas"]').val(data.kelas);
        $('[name="nilai_minimal"]').val(data.nilai_minimal);
  ...
  function simpanRef() {
    ...
    if (save_method == 'add') {
      url = "<?= base_url() . 'simanja/ref_kelas_jabatan/p_add_ref' ?>";
    } else {
      url = "<?= base_url() . 'simanja/ref_kelas_jabatan/p_update_ref' ?>";
    }
  ...
  function deleteRef(id) {
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            url: "<?= base_url() . 'simanja/ref_kelas_jabatan/delete_ref/' ?>" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
              $('#modalReferensi').modal('hide');
