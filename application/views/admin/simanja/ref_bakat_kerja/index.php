...
<div class="row bg-title">
  ...
  <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
    <h4 class="page-title">Referensi Bakat Kerja</h4>
  </div>
  <!-- /.page title -->
  <!-- .breadcrumb -->
  <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Ref. Bakat Kerja</li>
    </ol>
  </div>
  <!-- /.breadcrumb -->
</div>
      ...
      <div class="modal-body">
        <form id="formRef">
          ...
          <div class="form-group">
            ...
            <input type="text" name="kode" class="form-control" placeholder=" Masukkan Kode Bakat Kerja">
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">Arti</label>
            <input type="text" name="arti" class="form-control" placeholder=" Masukkan Arti Bakat Kerja">
          </div>
...
<script type="text/javascript">
  ...
  function editRef(id) {
    ...
    $('.help-block').empty();
    $.ajax({
      url: "<?= base_url() . 'simanja/ref_bakat_kerja/fetch_ref/' ?>" + id,
      type: "GET",
      dataType: "JSON",
  ...
  function simpanRef() {
    ...
    if (save_method == 'add') {
      url = "<?= base_url() . 'simanja/ref_bakat_kerja/p_add_ref' ?>";
    } else {
      url = "<?= base_url() . 'simanja/ref_bakat_kerja/p_update_ref' ?>";
    }
  ...
  function deleteRef(id) {
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            url: "<?= base_url() . 'simanja/ref_bakat_kerja/delete_ref/' ?>" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
              $('#modalReferensi').modal('hide');
