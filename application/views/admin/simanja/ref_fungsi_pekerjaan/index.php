...
<div class="row bg-title">
  ...
  <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
    <h4 class="page-title">Referensi Fungsi pekerjaan</h4>
  </div>
  <!-- /.page title -->
  <!-- .breadcrumb -->
  <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Ref. Fungsi pekerjaan</li>
    </ol>
  </div>
  <!-- /.breadcrumb -->
</div>
      ...
      <div class="modal-body">
        <form id="formRef">
          ...
          <div class="form-group">
            ...
            <input type="text" name="kode" class="form-control" placeholder=" Masukkan kode fungsi pekerjaan">
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">Keterangan</label>
            <input type="text" name="keterangan" class="form-control" placeholder=" Masukkan Keterangan fungsi pekerjaan">
          </div>
...
<script type="text/javascript">
  ...
  function editRef(id) {
    ...
    $('.help-block').empty();
    $.ajax({
      url: "<?= base_url() . 'simanja/ref_fungsi_pekerjaan/fetch_ref/' ?>" + id,
      type: "GET",
      dataType: "JSON",
  ...
  function simpanRef() {
    ...
    if (save_method == 'add') {
      url = "<?= base_url() . 'simanja/ref_fungsi_pekerjaan/p_add_ref' ?>";
    } else {
      url = "<?= base_url() . 'simanja/ref_fungsi_pekerjaan/p_update_ref' ?>";
    }
  ...
  function deleteRef(id) {
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            url: "<?= base_url() . 'simanja/ref_fungsi_pekerjaan/delete_ref/' ?>" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
              $('#modalReferensi').modal('hide');
