...
<div class="row">
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="panel panel-default" style="border-top: 10px solid #6003c8">
          ...
          <div class="panel-wrapper collapse in">
            ...
            <table id="w1" class="table table-striped"><tr><th width="180px" style="text-align:right">Nama Jabatan</th><td><?=$detail->nama ?: '-'?></td></tr>
                ...
                <tr><th width="180px" style="text-align:right">Ikhtisar Jabatan</th><td><?=$detail->ikhtisar_jabatan ?: '-'?></td></tr>
                <tr><th width="180px" style="text-align:right">Jumlah Pemangku Saat Ini</th><td><?=$detail->jumlah_pemangku ?: '-'?></td></tr>
                <tr><th width="180px" style="text-align:right">Jumlah Pemangku Berdasarkan Hasil ABK</th><td id="hasil_abk">-</td></tr>
                <tr><th width="180px" style="text-align:right">Kelebihan / Kekurangan</th><td id="kelebihan_kekurangan">-</td></tr>
              </table>
            </div>
          </div>
          ...
          <ul class="dropdown-menu">
              ...
              <li class="divider"></li>
              <li><a href="<?=base_url('simanja/analisis_jabatan/export_bkn/'.$detail->id)?>"><i class="fa fa-file-pdf-o"></i> <small>Export PDF BKN</small></a></li>
          </ul>
          <?php if($hasil_kerja){ ?>
            <a href="<?=base_url('simanja/analisis_beban_kerja/'.$detail->id)?>" class="btn btn-warning"><i class="fa fa-steam"></i> Analisis Beban Kerja</a>
          <?php } ?>
          <?php if($nilai > 50){ ?>
            <a href="<?=base_url('simanja/evaluasi_jabatan/'.$detail->id)?>" class="btn btn-success"><i class="fa fa-retweet"></i> Evaluasi Jabatan</a>
          <?php } ?>
        </div>
        </div>
      </div>
                        ...
                        ?>
                          <tr>
                              ...
                              <td style="width:150px">
                                  <a href="javascript:void(0)" onclick="editHasilKerja(<?=$i->id?>)" class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i></a>
                              </td>
                          </tr>
                      <?php } ?>
                      <tr>
        ...
        <div class="modal-body">
          <form id="formRef">
            ...
            <div class="form-group">
              ...
              <input type="number" name="jumlah_pemangku" class="form-control" placeholder="Masukkan Jumlah Pemangku Jabatan" <?=$disabled?>>
            </div>
        </div>
  ...
  <div id="modalHasilKerja" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        ...
        <div class="modal-body">
          <form id="formHasilKerja">
            ...
            <div class="form-group">
              <label for="message-text" class="control-label">Uraian Tugas <sup class="text-danger" title="wajib diisi">*</sup> </label>
              ...
              <div class="form-group">
                <label for="message-text" class="control-label">Hasil Kerja <sup class="text-danger" title="wajib diisi">*</sup> </label>
              ...
              <div class="form-group">
                <label for="message-text" class="control-label">Satuan Hasil <sup class="text-danger" title="wajib diisi">*</sup> </label>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
          <button type="button" onclick="simpanHasilKerja()" id="btnSaveHasilKerja" class="btn btn-primary">Simpan</button>
        </div>
        </form>
      </div>
      <!-- /.modal-content -->
...
<script>
    ...
    function simpanKualifikasiJabatan() {
      ...
      if(pendidikan_formal !== null && diklat_perjejangan !== null & diklat_teknis !== null && pengalaman_kerja !== null){
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.status) {
              ...
              $('#messageKualifikasiJabatan').html('<div class="alert alert-danger">' + data.message + '</div>');
            }
    ...
    function deleteKualifikasiJabatan(id) {
      swal({
        ...
        function(isConfirm) {
          if (isConfirm) {
            $.ajax({
              url: "<?= base_url() . 'simanja/analisis_jabatan/delete_kualifikasi_jabatan/' ?>" + id,
              type: "POST",
              dataType: "JSON",
              success: function(data) {
                $('#modalKualifikasiJabatan').modal('hide');
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.status) {
              ...
              $('#messageTugasPokok').html('<div class="alert alert-danger">' + data.message + '</div>');
            }
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.status) {
              ...
              $('#messageHasilKerja').html('<div class="alert alert-danger">' + data.message + '</div>');
            }
