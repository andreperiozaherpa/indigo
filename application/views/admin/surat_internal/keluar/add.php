<div class="container-fluid">
				...
				?>
					<div class="col-md-4">
						<div class="radio radio-primary">
							<input name="kop_surat" type="radio" id="radioks<?= $k ?>" value="<?= $s['value'] ?>" <?= ($s['value'] == "none") ? ' disabled' : '' ?><?= ($k == 0) ? ' checked' : '' ?>>
		...
		<div class="panel-body">
			...
			<div class="col-md-12 ">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="lampiran" placeholder="Masukan Daftar Lampiran" <?=$klk ? 'value="-"' :(($detail_cuti) ? 'value="-"' : '')?>>
				</div>
					...
					?>
						<div class="col-lg-4 col-md-2 col-sm-4 col-xs-12">
							<div class="radio radio-primary">
								...
								<label for="radioss<?= $k ?>"> <?= ucfirst($s) ?> </label><span class="badge badge-info pull-right" data-toggle="tooltip" data-placement="top" title="Tooltip on top"><i class="ti-info" style="font-size:9px"></i></span>
							</div>
						</div>
					<?php } ?>
				</div>
				...
				<div class="form-group" <?= $klk || $detail_cuti ? 'style="display:none"' : '' ?>>
					...
					<br>
					<a href="javascript:void(0)" onclick="selectAll()" class="btn btn-primary btn-xs">Pilih Semua Penerima</a>
					<br><br>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<div class="form-group">
						...
						<p><?=$klk->nama_kegiatan?></p>
					</div>
					<div class="form-group">
						<label><i class="ti-location-pin"></i> Lokasi Pengerjaan</label>
						<p><?=normal_string($klk->lokasi_pengerjaan)?></p>
					</div>
					<div class="form-group">
						<label>Tanggal Kegiatan</label>
						<p><?=tanggal($klk->tanggal_awal)?> <span style="font-weight:bold;color:#6003c8">s.d.</span> <?=tanggal($klk->tanggal_akhir)?></p>
					</div>
					<div class="form-group">
						<label>Deskripsi Kegiatan</label>
						<p><?=$klk->deskripsi_kegiatan?></p>	</div>
					<div class="form-group">
						<label>Target Kegiatan</label>
						<p><?=$klk->target_kegiatan?></p>
					</div>
						<?php
					}elseif($detail_cuti){
						?>
              ...
              <div class="row">
						...
						<div class="form-group">
							...
							<p><?=$detail_cuti->nama_lengkap?></p>
						</div>
						</div>
                <div class="col-md-6">
                  <label>Jenis Cuti</label>
                  <p><?= $detail_cuti->nama_jenis_cuti ?></p>
                </div>
                <div class="col-md-6">
                  <label>Tanggal Pengajuan</label>
                  <p><?= tanggal($detail_cuti->tanggal_pengajuan) ?></p>
                </div>
                <div class="col-md-6">
                  <label>Periode Cuti</label>
                  <p><?= tanggal($detail_cuti->tanggal_awal) ?><?= !empty($detail_cuti->tanggal_akhir) ? ' s.d. ' . tanggal($detail_cuti->tanggal_akhir) : null ?></p>
                </div>
                <div class="col-md-6">
                  <label>Alamat Selama Menjalankan Cuti</label>
                  <p><?= $detail_cuti->alamat ?></p>
                </div>
                <div class="col-md-12">
                  <label>Keterangan</label>
                  <p><?= $detail_cuti->keterangan ?></p>
                </div>
              </div>
						<?php
					} else {
							...
							<div class="form-group">
								<?php if ($f->input_type !== 'header') { ?>
									...
									<?php if ($f->input_type == 'text' || $f->input_type == 'date' || $f->input_type == 'number' || $f->input_type == 'file') { ?>
										<input type="<?= $f->input_type ?>" class="<?= $f->field_class ?>" name="<?= $f->field_name ?>" placeholder="<?= empty($f->field_placeholder) ? 'Masukkan ' . $f->field_label : $f->field_placeholder ?>">
											...
											<select class="<?= $f->field_class ?>" name="<?= $f->field_name ?>">
												...
												<select class="<?= $f->field_class ?> select2" name="<?= $f->field_name ?>">
													...
													?>
														...
														<?php } else { ?>
															<label> </label>
															<<?= $f->field_subtype ?> style="font-size:20px"><?= $f->field_label ?>
															</<?= $f->field_subtype ?>>
														<?php } ?>
							</div>
						<?php } ?>
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<small><span style="font-weight: bold;color: red;">Catatan</span> : Ekstensi yang diizinkan adalah <b>doc, docx, pdf, xls, xlsx, ppt, pptx, txt, zip, rar</b> dengan Maksimal ukuran file <b>2MB</b></small><br><br>
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			<div class="row">
				<div class="col-md-6">
					<div class="form-group">
						<label class="col-md-12">Pemeriksa <span class="text-danger">*</span> <small>(Atasan Langsung)</small></label>
	...
	<div class="pull-right">
		<a href="<?php echo base_url('surat_internal/kategori_surat_keluar'); ?>" class="btn btn-default btn-outline"><i class="ti-arrow-circle-left"></i> Kembali</a>
	...
	<script type="text/javascript">
		function getUnitkerja() {
			...
			$.post('<?= base_url() ?>surat_internal/get_unit_kerja', {
				id_skpd: id_skpd
			}, function(data) {
		...
		function getPegawai() {
			...
			$.post('<?= base_url() ?>surat_internal/get_pegawai', {
