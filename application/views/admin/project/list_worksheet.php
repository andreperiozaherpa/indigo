 ...
 <div class="container-fluid">
                            ...
                            <div class="row">
                                <div class="col-md-5">
                                   ...
                                   <table class="table datatable" id="data">
                                    <h3> &nbsp;<b class="text-danger"><?php echo $pr->project_name ?></b><a href="<?php echo base_url('manage_project/edit_project/'.$pr->project_id); ?>" class="fcbtn btn btn-xs btn-info btn-outline btn-1c pull-right"><i class="fa fa-edit"></i> Edit</a></h3>
                                                ...
                                                <tr id="add-ws-<?php echo $no_ws;?>">
                                                    ...
                                                    <td>
                                                        <select name="member<?php echo $no_ws;?>[]" id="member-<?php echo $no_ws;?>" class="select2 m-b-10 select2-multiple" multiple="multiple" data-placeholder="Choose" <?php echo $approved;?>>
                                                            <?php foreach ($employee as $key): $array_team = explode(',', $pr->team); $selecting = (array_search($key->employee_id, $array_team) === false ) ? '' : 'selecting'; $array_member = explode(',', $ws->member); $selected = (array_search($key->employee_id, $array_member) === false ) ? '' : 'selected'?>
                                                                <?php if (!empty($selecting)) echo "<option value='{$key->employee_id}' {$selected}>{$key->employee_name}</option>" ?>
                                                            <?php endforeach ?>
                                                        </select>
                                                    </td>
    ...
    <?php 
        ...
        foreach ($employee as $key){ 
            $selecting = (array_search($key->employee_id, $array_team) === false ) ? '' : 'selecting';
            if (!empty($selecting)) $member_option .= "<option value='{$key->employee_id}' {$selected}>{$key->employee_name}</option>";
        } 
    ?>
