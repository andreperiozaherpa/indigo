	...
	<body>
		...
		<table class="table" border=<?=$border;?> style="border-collapse:collapse" width='100%'>
			<tr align=center>
				...
				<td rowspan=2 colspan=<?=$colspan;?>>JABATAN</td>
			...
			<?php
				...
				foreach($data as $row)
				{
					...
					if ($row['level_jabatan']==1) $total_fungsional += $FUNGSIONAL;
					$LAIN2 = $row['LAIN2'];
					if ($row['level_jabatan']==1) $total_lain2 += $LAIN2;
					$PELAKSANA= $row['PELAKSANA'];
					if ($row['level_jabatan']==1) $total_pelaksana += $PELAKSANA;
					$IA = $row['IA'];
					if ($row['level_jabatan']==1) $TOTAL_IA += $IA;
					$IB = $row['IB'];
					if ($row['level_jabatan']==1) $TOTAL_IB += $IB;
					
					$IIA = $row['IIA'];
					if ($row['level_jabatan']==1) $TOTAL_IIA += $IIA;
					$IIB = $row['IIB'];
					if ($row['level_jabatan']==1) $TOTAL_IIB += $IIB;
					
					$IIIA = $row['IIIA'];
					if ($row['level_jabatan']==1) $TOTAL_IIIA += $IIIA;
					$IIIB = $row['IIIB'];
					if ($row['level_jabatan']==1) $TOTAL_IIIB += $IIIB;
					
					$IVA = $row['IVA'];
					if ($row['level_jabatan']==1) $TOTAL_IVA += $IVA;
					$IVB = $row['IVB'];
					if ($row['level_jabatan']==1) $TOTAL_IVB += $IVB;
					
					$VA = $row['VA'];
					if ($row['level_jabatan']==1) $TOTAL_VA += $VA;
					$VB = $row['VB'];
					if ($row['level_jabatan']==1) $TOTAL_VB += $VB;
					
					$SUB_TOTAL_STRUKTURAL = $IA + $IB + $IIA + $IIB + $IIIA + $IIIB + $IVA + $IVB + $VA + $VB;
					$SUB_TOTAL = $FUNGSIONAL + $SUB_TOTAL_STRUKTURAL;
					
					if ($row['level_jabatan']==1) $TOTAL_STRUKTURAL += $SUB_TOTAL_STRUKTURAL;
