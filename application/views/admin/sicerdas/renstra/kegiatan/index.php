        ...
        <div class="modal-body">
          <form id="form-data" action="#!">
           ...
           <div class="form-group">
            <label for="id_program_renstra" class="control-label">Program / Nama Sasaran Renstra</label>
            <select id="id_program_renstra" name="id_program_renstra" class="form-control select2 input_select" onchange="get_indikator()" >
             ...
             <?php foreach($dt_program_renstra as $row){
              echo '<option value="'.$row->id_program_renstra.'">'.$row->kode_program.' - '.$row->nama_program.' / '.$row->nama_sasaran_renstra.'</option>';
            }
            ?>
            </select>
...
<script type="text/javascript">
  ...
  function loadPagination(page_num) {
   ...
   function edit(i)
   {
      ...
      action = "edit";
      id_kegiatan = rowData[i].id_kegiatan;
      reset_error();
      id_indikator_program_renstra = rowData[i].id_indikator_program_renstra;
      id_ref_kegiatan = rowData[i].id_ref_kegiatan;
      $("#id_program_renstra").val(rowData[i].id_program_renstra).trigger("change");
   ...
   function get_indikator()
   {
    ...
    function get_kegiatan()
    {
      ...
      $.ajax({
         ...
         data: {
            id_program_renstra : $("#id_program_renstra").val(),
            id_ref_kegiatan : id_ref_kegiatan,
         },
         success: function (data) {
            $("#id_ref_kegiatan").html(data.content).trigger("change");
         },
         error: function (xhr, status, error) {
            console.log(xhr.responseText);
            swal("Opps", "Terjadi kesalahan", "error");
    ...
    function get_unit_kerja()
    {
      ...
      if(id_program_renstra != "")
      {
        ...
        $.ajax({
           url: "<?=base_url()?>sicerdas/renstra/kegiatan/get_unit_kerja/",
           type: 'post',
           dataType: 'json',
           data: {
            id_program_renstra : $("#id_program_renstra").val(),
