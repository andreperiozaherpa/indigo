                  ...
                  <form method="POST">
										<div class="col-md-10">
											<div class="col-md-3" >
												<div class="row">
													<div class="form-group">
														...
														<option value="<?=$jps->id_ref_jenis_pengajuan_surat?>" <?php if(!empty($jenis)) { echo ($jps->id_ref_jenis_pengajuan_surat == $jenis) ? 'selected' : null; }?>><?=$jps->jenis_pengajuan_surat?></option>
														<?php } ?>
														</select>
													</div>
												</div>
											...
											<div class="col-md-3" >
												<div class="row">
													<div class="form-group">
														<label>Status Pengajuan Surat</label>
														<select class="form-control" name="status" id="" style="width:95%">
														<option value="">--Pilih--</option>
														<option value="Sudah Diverifikasi" <?php if(!empty($status)) { echo ('Sudah Diverifikasi' == $status) ? 'selected' : null; }?>>Sudah Diverifikasi</option>		
														<option value="Belum Diverifikasi" <?php if(!empty($status)) { echo ('Belum Diverifikasi' == $status) ? 'selected' : null; }?>>Belum Diverifikasi</option>		
														</select>
													</div>
												</div>
											</div>
					...
					<div class="mail-status info" style="background-color: <?=$color?>">
						...
						<div class="white-box body">
							...
							<div class="mail-footer"><span class="label label-primary pull-left" style="background-color: #fff;color: #6003C8;border:solid 1px #6003C8;border-radius: 0;"><?=tanggal(date('Y-m-d',strtotime($pj->created_at)))?></span>
								<a href="https://e-office.sumedangkab.go.id/verifikasi_pengajuan_surat/detail/<?=$pj->id_pengajuan_surat?>" class="btn btn-primary pull-right"><i class="ti-arrow-circle-right"></i> Detail Pengajuan</a>
							</div>
						</div>
					</div>
				</div>
