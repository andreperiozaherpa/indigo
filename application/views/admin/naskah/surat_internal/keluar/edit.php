                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="surat_klasifikasi" class="classification" id="classification" class="form-control" required>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="nomer_surat" id="nomer_surat" value="<?=$detail->nomer_surat?>" class="form-control" placeholder="Masukan No. Surat" required>
                                    </div>
                                </div>
                            </div>

<!--                            <div class="form-group">-->
<!--                                <label>Nomer Surat Lengkap <span class="text-danger">*</span></label>-->
<!--                                <input type="text" name="nomer_surat_lengkap" value="--><?//=$detail->nomer_surat_lengkap;?><!--" class="form-control" placeholder="Masukan Nomer Surat Lengkap" required>-->
<!--                            </div>-->
								...
								<select multiple id="optgroup" name="id_penerima[]">
									...
									foreach($pegawai as $p){
										...
										foreach($penerima as $pp){
											if($p->id_pegawai==$pp->id_pegawai){
												$selected[] = $p->id_pegawai;
											}
										}

										if(in_array($p->id_pegawai, $selected)){
											$ss = ' selected';
										}else{
											$ss = '';
										}
										echo '<option value="'.$p->id_pegawai.'"'.$ss.'>'.$p->nama_lengkap.' - '.$p->jabatan.'</option>';
									}
									?>
								</select>
							</div>
				...
				if($id_ref_surat!=='custom'){
						...
						<div class="panel-body">
							<?php foreach($field as $f) {
												...
												<select class="<?=$f->field_class?>" name="<?=$f->field_name?>">
													...
													<select  onchange="getKecamatanPemohon<?=$f->id_field?>()" id="<?=$f->r_table?><?=$f->id_field?>" class="<?=$f->field_class?>" name="<?=$f->field_name?>">
														...
														<select  onchange="getKabupatenPemohon<?=$f->id_field?>()" id="<?=$f->r_table?><?=$f->id_field?>" class="<?=$f->field_class?>" name="<?=$f->field_name?>">
															...
															?>
															<select <?=$ajax?> id="<?=$f->r_table.$f->id_field?>" class="<?=$f->field_class?> select2" name="<?=$f->field_name?>">
																<option value="">-- Pilih <?=$f->field_label?> --</option>
																<?php
								...
								<div class="pull-right">
									<a href="<?php echo base_url('surat_internal/detail_surat_keluar/'.$detail->id_surat_keluar);?>" class="btn btn-default btn-outline"><i class="ti-arrow-circle-left"></i> Kembali</a>
							...
							<script type="text/javascript">
								...
								function getUnitkerja(){
									...
									$.post('<?=base_url()?>surat_internal/get_unit_kerja', {id_skpd:id_skpd}, function(data){ 
				  ...
				  function getPegawai(){
				  	...
				  	$.post('<?=base_url()?>surat_internal/get_pegawai', {id_unit_kerja:id_unit_kerja}, function(data){ 
    ...
    <script type="text/javascript">
        var klasifikasi = <?php echo json_encode($classification->id_surat_klasifikasi); ?>;
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                type: 'GET',
                url: '<?= base_url("arsip_dinamis/berkas_aktif/get_classification?suratKlasifikasi=") ?>' + klasifikasi
            }).then(function (data) {
                var dao = JSON.parse(data);

                // create the option and append to Select2
                var option = new Option(dao.nama_klasifikasi, dao.id_surat_klasifikasi, true, true);
