...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-md-12">
      <a href="<?= base_url('naskah/surat_internal/surat_keluar') ?>" class="pull-right btn btn-primary btn-outline"><i
            ...
            <div class="panel panel-primary">
              <div class="panel-body" style="border-top: solid 5px #6003C8">
                    ...
                    foreach ($penerima as $p) {
                      ...
                      <div style="margin-bottom:10px;border: solid 1px #cdcdcd;text-align: left !important;padding:4px">
                        ...
                        <small style="display: block"><i style="color: #5D03C1" class="ti-bar-chart"></i>
                          <?= ($p->id_unit_kerja > 0) ? $p->nama_jabatan : "Kepala SKPD" ?>
                        </small>
                      </div>
                    <?php } ?>
                    <span class="badge" style="background-color: grey;font-size:10px;">
                ...
                if ($detail->status_ttd == "sudah_ditandatangani") {
                  ?>
                  <a href="<?= base_url('data/surat_internal/ttd/' . $detail->file_ttd . '') ?>"
                    class="btn btn-primary btn-block btn-outline"><i class="ti-download"></i> Download Surat</a>
                  <?php
                } else {
                  ?>
                  <a href="<?= base_url('data/surat_internal/keluar/' . $detail->file_draft_surat . '') ?>"
                    class="btn btn-primary btn-block btn-outline"><i class="ti-download"></i> Download Surat</a>
                <?php } ?>
                <?php
                if ($detail->status_verifikasi !== 'sudah_diverifikasi') {
                  ...
                  <a href="<?= base_url('naskah/surat_internal/edit_surat_keluar/' . $detail->id_ref_surat . '/' . $detail->id_surat_keluar) ?>"
                ...
                if ($status_surat == 'belum_diupload') {
                  ?>
                  <a href="<?= base_url('naskah/surat_internal/buat_ulang_surat/' . $detail->id_surat_keluar) ?>"
              ...
              <div class="panel panel-primary">
                <div class="panel-body">
                  ...
                  <div class="text-center">
                    ...
                    <a target="blank"
                      href="<?= base_url('data/surat_internal/lampiran/' . $detail->file_lampiran . '') ?>"
    ...
    <div class="col-md-9">
      ...
      <div class="white-box">
        ...
        <div class="row">
          ...
          if ($detail->status_ttd == "sudah_ditandatangani") {
            ?>
            <a href="<?= base_url('data/surat_internal/ttd/' . $detail->file_ttd . '') ?>" class="btn btn-primary"
            ...
            <div class="col-lg-4 col-md-5 col-sm-6 btn-margin-top">
              <a href="<?= base_url('data/surat_internal/keluar/' . $detail->file_draft_surat . '') ?>"
                class="btn btn-primary btn-block" type="button"><span class="btn-label"><i
                    class="ti-cloud-down"></i></span> Download Draf Surat</a>
            </div>
            <?php
            ...
            <?php } else {
              ?>
              <div class="col-lg-4 col-md-5 col-sm-6 btn-margin-top">
                <a href="javascript:void(0)" class="btn btn-default btn-outline btn-block" type="button" data-toggle="modal"
                  data-target="#myModal"><span class="btn-label"><i class="ti-cloud-up"></i></span>
                  <?= empty($detail->file_verifikasi) || is_null($detail->file_verifikasi) ? "Upload" : "Upload Ulang" ?>
                  Draf Surat
                </a>
              </div>
              <?php
    ...
    function deleteSurat(id) {
      swal({
        ...
        function (isConfirm) {
          if (isConfirm) {
            $.ajax({
              url: "<?= base_url() . 'naskah/surat_internal/delete_surat_keluar/' ?>" + id,
              type: "POST",
              dataType: "JSON",
              success: function (data) {
                swal("Berhasil", "Data Berhasil Dihapus!", "success");
                // location.reload();
                window.location.href = "<?= base_url() . 'naskah/surat_internal/surat_keluar' ?>";
              },
