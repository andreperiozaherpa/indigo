...
<div class="container-fluid">
  ...
  <div class="row bg-title">
    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
      <h4 class="page-title">Verifikasi Surat eksternal</h4>
    </div>
  ...
  <div class="col-md-12">
    <a href="<?= base_url('naskah/surat_eksternal/verifikasi_surat') ?>"
      class="pull-right btn btn-primary btn-outline"><i class="ti-back-left"></i> Kembali</a><br><br>
  </div>
  <div class="col-md-3">
    <div class="row">
              ...
              <div class="panel-body" style="border-top: solid 5px #6003C8">
                    ...
                    <p>
                      <span class="text-<?= $color1 ?>">
                        <i style="background-color: <?= $color2 ?>;border-radius: 50%;color: #fff;padding: 5px;"
                          class="<?= $icon2 ?>"></i>
                        <?= normal_string($detail->status_verifikasi) ?>
                      </span>
                    </p>
                  </div>
                </div>
                <div class="row">
                  <div class="col-md-12 text-center">
                    ...
                    <span class="badge" style="background-color: grey;font-size:10px;">
              ...
              <div class="panel panel-primary">
                <div class="panel-body">
                  ...
                  <div class="text-center">
                    ...
                    <a target="blank"
                      href="<?= base_url('data/surat_eksternal/lampiran/' . $detail->file_lampiran . '') ?>"
  ...
  <div class="col-md-9">
    <div class="row">
      <div class="panel panel-primary">
        <div class="panel-body" style="border-top: solid 5px #6003C8">
          <h3 style="color: #6003C8" class="box-title">
            <?= $detail->nama_surat ?>
          </h3>
          <br>
          <div class="col-md-6">
        ...
        <div class="panel-footer">
          <form method="POST">
            ...
            if ($detail->status_verifikasi == 'sudah_diverifikasi' or $detail->id_pegawai_verifikasi != $this->session->userdata('id_pegawai')) {
              ?>
              <button class="btn btn-primary" type="button" disabled><span class="btn-label"><i
                    class="ti-check"></i></span> Verifikasi</button>
              <button class="btn btn-success" type="button" disabled data-toggle="modal" data-target="#mdverifikasi"><span
                  class="btn-label"><i class="ti-shift-right"></i></span>Teruskan</button>
              <button class="btn btn-default btn-outline" type="button" disabled><span class="btn-label"><i
                    class="ti-back-left"></i></span>Tolak</button>
            <?php } else {
              ?>
              <button class="btn btn-primary" type="submit" name="terima"><span class="btn-label"><i
                    class="ti-check"></i></span> Verifikasi</button>
              <button class="btn btn-success" type="button" data-toggle="modal" data-target="#mdverifikasi"><span
                  class="btn-label"><i class="ti-shift-right"></i></span>Teruskan</button>
              <button class="btn btn-default btn-outline" type="button" data-toggle="modal" data-target="#mdTolak"><span
                  class="btn-label"><i class="ti-back-left"></i></span>Tolak</button>
              <?php
            } ?>
      ...
      <?php if ($detail_verifikasi): ?>
        <div class="white-box">
          <div class="row" style="margin-bottom: 15px">
            <span style="float: left">
              <h5 class="box-title"><i style="color:#fff;background-color: #6003C8;padding: 5px;border-radius: 50%"
                  class="ti-medall"></i> <span style="border-bottom: solid 2px #6003C8">Sudah disetujui juga oleh</span>
              </h5>
            </span>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="row">
                <?php foreach ($detail_verifikasi as $key2 => $row):
                  $row->foto_pegawai = ($row->foto_pegawai == '') ? 'user-default.png' : $row->foto_pegawai; ?>
                  <div class="col-md-12 m-t-5" id="selected-disposisi-internal-<?= $key2 ?>-<?= $row->id_pegawai ?>"
                    style="margin-bottom:10px;border: solid 1px #6003c8;text-align: left !important;padding:4px">
                    <div class="col-md-4">
                      <img src="<?= base_url('data/foto/pegawai/' . $row->foto_pegawai) ?>" alt="user"
                        class="img-circle img-responsive" style="max-height: 75px;">
                    </div>
                    <div class="col-md-8">
                      <small style="display: block" class="text-purple">
                        <?= $row->nama_lengkap ?> <span class="label label-rouded label-primary">
                      ...
                      <span class="well" style="padding: unset;">
                        <?= $row->tgl_verifikasi ?>
                      </span>
                    </div>
                  </div>
      ...
      <div class="white-box">
        ...
        <div class="alert alert-<?= $a_type ?>">
          ...
          if ($detail->status_verifikasi == 'ditolak') {
            ?><br>
            Alasan Penolakan :
            <?= $detail->alasan_penolakan_verifikasi ?>
            <?php
          }
          ?>
        </div>
        ...
        if ($detail->status_ttd == "sudah_ditandatangani") {
          ...
          <a href="<?= base_url('data/surat_eksternal/keluar/' . $detail->file_draft_surat . '') ?>"
            class="btn btn-primary" type="button"><span class="btn-label"><i class="ti-cloud-down"></i></span> Download
            Draf Surat</a>
          <?php
          if ($detail->status_verifikasi == 'sudah_diverifikasi' or $detail->id_pegawai_verifikasi != $this->session->userdata('id_pegawai')) {
            ?>
        ...
        <div class="modal-body">
          <form method="POST">
            <div class="form-group">
              ...
              <textarea class="form-control" name="alasan_penolakan_verifikasi"
  ...
  <div id="mdverifikasi" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST">
            <div class="form-group">
              ...
              <select id="id_pegawai" name="id_pegawai_verifikasi_teruskan" class="select2 m-b-10 form-control"
                data-placeholder="Pilih Verifikator" required>
                ...
                <?php endforeach ?>
                <?php
                if ($detail->kop_surat == 'bupati') {
                  ?>

                  <optgroup label="Sekretaris Daerah">
                    <option value="<?= $pegawai1->id_pegawai ?>"><?= $pegawai1->nama_lengkap ?> - <?= $pegawai1->jabatan ?></option>
                  </optgroup>
                  <?php
                }
                ?>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>
          <button class="btn btn-primary" type="submit" name="teruskan"><span class="btn-label"><i
                class="ti-shift-right"></i></span>Setujui & Teruskan</button>
          </form>
        </div>
      </div>
