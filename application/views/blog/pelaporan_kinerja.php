...
<section class="banner_area" style="min-height: unset;">
			<div class="banner_inner d-flex align-items-center">
				...
				<h2 style="margin-top:5px">Pelaporan Kinerja, SEMONGKO !</h2>
					</div>
				</div>
			</div>
	...
	<div class="container">
		<div class="add-listing-section margin-top-45">
			...
			<div class="add-listing-headline">
				<h3><i class="sl sl-icon-book-open"></i> Pelaporan Kinerja <?=$tahun_?> <div class="pull-right"><select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" class=""><?php
        $current_year = date("Y");
        $array_year = array();
        foreach($tahun as $r){
          if ($r->tahun_berkas>0) {
        ...
        foreach ($array_year as $year) {
          ...
          echo'<option value="'.base_url("pelaporan_kinerja/index/".$year).'" '.$selected.'>'.$year.'</option>';
        }
        ?></select></div></h3>
			</div>
							...
							<table style="margin-top: 20px" class="basic-table" >
								<thead>
									<tr>
										<th width="7%">NO</th>
										<th>NAMA SKPD</th>
										<th width="7%" style="text-align: center; ">LK</th>
									</tr>
								</thead>
								<tbody>
									<?php $no=1; foreach($list as $uk){
										?>
										<tr class="odd gradeX" id="tr_1">
											...
											<td style="text-align: center; ">
												<?php 
												foreach ($berkas as $b) { ?>
													<?php if ($b->nama_skpd == $uk->nama_skpd):?><a href="<?=base_url('data/berkas_lakip/'.$b->lkj.'')?>" target="_blank" class="btn btn-info btn-sm" download><i class="fa fa-download"></i> Unduh</a><?php endif;?>
												<?php }
												?>
											</td>
											</tr>
