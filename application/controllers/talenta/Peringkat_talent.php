...
class Peringkat_talent extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->ids_inject = [561, 546, 667, 666, 322, 592, 564, 646, 628, 91, 369, 409, 547, 367, 555, 548, 562, 155, 544];
	}
	...
	public function ranking_guru()
	{
		if ($this->user_id) {
			$data['title'] = "Peringkat Talent Guru - Admin ";
			$data['content'] = "talenta/peringkat_talent/simpeg_guru";
			$data['user_picture'] = $this->user_picture;
			$data['full_name'] = $this->full_name;
			$data['user_level'] = $this->user_level;
			$data['active_menu'] = 'talenta/peringkat_talent/simpeg';
			$data['rumpun'] = array('Administrasi', 'Ekonomi', 'Hukum', 'KePU-AN', 'Kesehatan', 'Kesejahteraan', 'Lingkungan Hidup', 'Pemerintahan', 'Pertanian');
			$data['pangkat'] = array('Guru');
			$data['golongan'] = array('I/a', 'I/b', 'I/c', 'I/d', 'II/a', 'II/b', 'II/c', 'II/d', 'III/a', 'III/b', 'III/c', 'III/d', 'IV/a', 'IV/b', 'IV/c', 'IV/d', 'IV/e');
			$data['pendidikan'] = array('SD / Sederajat', 'SMP / Sederajat', 'SMA / Sederajat', 'D1', 'D2', 'D3', 'S1', 'S2', 'S3');
			$data['eselon'] = array('Guru');
			$data['hasil_asesmen'] = array('perlu_pengembangan', 'perlu_pengembangan_lebih_lanjut', 'sesuai', 'sesuai_dengan_pengembangan');
			$data['diklat_pim'] = array('I', 'II', 'III', 'IV');
			$data['masa_kerja'] = array('< 3' => 'Kurang dari 3 tahun', '3 s.d 5' => '3 sampai 5 tahun', '> 5' => 'Lebih dari 5 tahun');
			$rumpun = isset($_GET['rumpun']) ? $_GET['rumpun'] : '';
			$eselon = isset($_GET['eselon']) ? $_GET['eselon'] : '';
			$fkuadran = isset($_GET['kuadran']) ? $_GET['kuadran'] : '';
			$list = $this->peringkat_talent_model->get_all_talent_guru($rumpun, $eselon);
			$list_all = $this->peringkat_talent_model->get_all_talent_guru($rumpun, $eselon);
			$data['nilai_kuadran'] = get_nilai_kuadran($eselon);
			// print_r($list);
			// die;
			if (!empty($list)) {
	...
	public function get_talent($offset = 0, $nip = null)
	{
		...
		if ($nip != null and strlen($nip) == 18) {
			...
			$this->db->select("nip,id_pegawai,ref_skpd.id_skpd,ref_skpd.jenis_skpd");
			...
			// $this->db->or_where('ref_skpd.id_skpd', '1817');
			// $this->db->or_where('ref_skpd.jenis_skpd', 'sekolah');
			// $this->db->group_end();
			$this->db->group_end();
			...
			$get = $query->result();
			// print_r($get);
			echo "TOTAL DATA = " . count($get) . " OFFSET(" . $offset . ")\n";
			// die;
			foreach ($get as $key => $value) {

				if (in_array($value->id_pegawai, $this->ids_inject)) {
					continue;
				}

				$no = $key + 1;
				$nip[] = $value->nip;
				echo "INPUT DATA NO." . $no . " -> NIP :" . $value->nip . " - " . date('Y-m-d H:i:s');
				echo "\n";
				$jenis_skpd_pegawai = ($value->id_skpd == '1817' or $value->jenis_skpd == 'sekolah') ? "Guru" : "Struktural";
				$this->generate($value->nip, $value->id_pegawai, $jenis_skpd_pegawai);
				// echo "$value->nip <br>";
			}
			// $this->db->select("nip,id_pegawai");
	...
	public function generate($nip = "", $id = "", $jenis_skpd_pegawai = "Struktural")
	{
		if (!$this->user_id) {
		...
		$data['jenis_skpd_pegawai'] = $jenis_skpd_pegawai;
	...
	function operate($data, $id)
	{
		...
		foreach ($pegawai as $key => $value) {
			...
			$data = array(
				...
				"jenis_skpd_pegawai" => $data['jenis_skpd_pegawai'],
			);
	...
	function update_posisi_box($nip = null, $output = true)
	{
		if (!empty($nip)) {
			...
			$data_potensi = $this->db->select('nilai_potensi')->where('tahun', date('Y'))->like('eselon', 'II.', 'after')->where_not_in('nip', $nip_guru)->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result_array();
			$data_potensi = array_map('current', $data_potensi);

			$data_kompetensi = $this->db->select('nilai_kompetensi')->where('tahun', date('Y'))->like('eselon', 'II.', 'after')->where_not_in('nip', $nip_guru)->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result_array();
			$data_kompetensi = array_map('current', $data_kompetensi);

			$pegawai_talent = $this->db->select('id_pegawai, id_pegawai_talent, nilai_potensi, nilai_kompetensi')->where('tahun', date('Y'))->like('eselon', 'II.', 'after')->where_not_in('nip', $nip_guru)->where_not_in('id_pegawai', $this->ids_inject)->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result();
			...
			$data_potensi = $this->db->select('nilai_potensi')->where('tahun', date('Y'))->group_start()->like('eselon', 'III.', 'after')->or_where_in('nip', $nip_guru)->group_end()->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result_array();
			$data_potensi = array_map('current', $data_potensi);

			$data_kompetensi = $this->db->select('nilai_kompetensi')->where('tahun', date('Y'))->group_start()->like('eselon', 'III.', 'after')->or_where_in('nip', $nip_guru)->group_end()->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result_array();
			$data_kompetensi = array_map('current', $data_kompetensi);

			$pegawai_talent = $this->db->select('id_pegawai, id_pegawai_talent, nilai_potensi, nilai_kompetensi')->where('tahun', date('Y'))->group_start()->like('eselon', 'III.', 'after')->or_where_in('nip', $nip_guru)->group_end()->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result();
			...
			$data_potensi = $this->db->select('nilai_potensi')->where('tahun', date('Y'))->group_start()->or_not_like('eselon', 'II.', 'both')->or_where('eselon', null)->group_end()->where_not_in('nip', $nip_guru)->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result_array();
			$data_potensi = array_map('current', $data_potensi);

			$data_kompetensi = $this->db->select('nilai_kompetensi')->where('tahun', date('Y'))->group_start()->or_not_like('eselon', 'II.', 'both')->or_where('eselon', null)->group_end()->where_not_in('nip', $nip_guru)->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result_array();
			$data_kompetensi = array_map('current', $data_kompetensi);

			$pegawai_talent = $this->db->select('id_pegawai, id_pegawai_talent, nilai_potensi, nilai_kompetensi')->where('tahun', date('Y'))->group_start()->or_not_like('eselon', 'II.', 'both')->or_where('eselon', null)->group_end()->where_not_in('nip', $nip_guru)->where('jenis_skpd_pegawai', 'Struktural')->get('pegawai_talent_simpeg')->result();
			...
			$data_potensi = $this->db->select('nilai_potensi')->where('tahun', date('Y'))->where('jenis_skpd_pegawai', 'Guru')->get('pegawai_talent_simpeg')->result_array();
			$data_potensi = array_map('current', $data_potensi);

			$data_kompetensi = $this->db->select('nilai_kompetensi')->where('tahun', date('Y'))->where('jenis_skpd_pegawai', 'Guru')->get('pegawai_talent_simpeg')->result_array();
			$data_kompetensi = array_map('current', $data_kompetensi);
			$pegawai_talent = $this->db->select('id_pegawai, id_pegawai_talent, nilai_potensi, nilai_kompetensi')->where('tahun', date('Y'))->where('jenis_skpd_pegawai', 'Guru')->get('pegawai_talent_simpeg')->result();
			...
			echo "TOTAL UPDATING BOX {$i}\n";
		}
	}

	function get_perilaku($nip)
	{
		//$rating = $this->peringkat_talent_model->get_k_peer($id_pegawai);
	...
	public function inject_pegawai()
	{

		$this->update_posisi_box(null, false);
		$max_nilai = 100;
		$bobot_assestment = 19; //15;
		$bobot_pendidikan = 19; //15;
		$bobot_masa_kerja = 19; //15;
		...
		$this->db->where('posisi_box', 9);
		$this->db->where('tahun', 2023);
		$this->db->like('eselon', 'III.');
		$select_query = '';
		foreach ($list_skor as $l) {
			$select_query .= "MIN($l) as min_$l, MAX($l) as max_$l, AVG($l) as avg_$l,";
		}
		$this->db->select($select_query);
		$pegawai_box_9 = $this->db->get('pegawai_talent_simpeg')->row();
		foreach ($list_skor as $l) {
			$nilai_box_9[$l]['avg'] = $pegawai_box_9->{'avg_' . $l};
			$nilai_box_9[$l]['min'] = $pegawai_box_9->{'min_' . $l};
			$nilai_box_9[$l]['max'] = $pegawai_box_9->{'max_' . $l};
		}
		...
		$this->db->where_in('pegawai_talent_simpeg.id_pegawai', $ids);
		$this->db->join('pegawai', 'pegawai.id_pegawai = pegawai_talent_simpeg.id_pegawai');
		$this->db->where('tahun', 2023);
		$this->db->where('posisi_box <', 9);
		$pegawai = $this->db->get('pegawai_talent_simpeg')->result();

		$n = 1;
		foreach ($pegawai as $p) {
			echo "[$n] $p->nama_lengkap $p->posisi_box";
			$update_data = [];
			foreach ($list_skor as $l) {
				$$l = $p->$l;
				if ($l == 'skor_wawancara' || $l == 'skor_tpp' || $l == 'skor_masa_kerja') {
					continue;
				}
				if ($l == 'skor_lkh' || $l == 'skor_peer') {
					$max_nilai = 5;
					$rand = rand(0.1 * 100, 0.5 * 100) / 100;
					$generated_nilai = $nilai_box_9[$l]['max'] - $rand;
				} else {
					$max_nilai = 100;
					$rand = rand(5, 15);
					$generated_nilai = $nilai_box_9[$l]['min'] + $rand;
				}
				$skor = $p->$l;

				$persentase = ($skor / $max_nilai) * 100;
				if ($skor < $nilai_box_9[$l]['avg']) {
					echo "$l kurang dari " . $nilai_box_9[$l]['min'] . " : $skor -> ";
					if ($generated_nilai > $nilai_box_9[$l]['max']) {
						$generated_nilai = $nilai_box_9[$l]['max'];
					}
					$update_data[$l] = $generated_nilai;
					$$l = $generated_nilai;
					echo $generated_nilai . " ";

				}
			}

			if (!empty($update_data)) {
				$this->db->where('id_pegawai_talent', $p->id_pegawai_talent);
				$this->db->update('pegawai_talent_simpeg', $update_data);
			}
			...
			$nilai_potensi = round($nilai_potensi, 2);
			$nilai_kompetensi = round($nilai_kompetensi, 2);


			$update_data2 = [
				'nilai_assestment' => $nilai_assestment,
				'nilai_pendidikan' => $nilai_pendidikan,
				'nilai_masa_kerja' => $nilai_masa_kerja,
				'nilai_jabatan' => $nilai_jabatan,
				'nilai_pelatihan' => $nilai_pelatihan,
				'nilai_wawancara' => $nilai_wawancara,
				'nilai_potensi' => $nilai_potensi,
				'nilai_ppk_pns' => $nilai_ppk_pns,
				'nilai_prestasi' => $nilai_prestasi,
				'nilai_penugasan' => $nilai_penugasan,
				'nilai_peer' => $nilai_peer,
				'nilai_tpp' => $nilai_tpp,
				'nilai_lkh' => $nilai_lkh,
				'nilai_kompetensi' => $nilai_kompetensi,
				'nilai_talent' => $nilai_talent,
			];

			$this->db->where('id_pegawai_talent', $p->id_pegawai_talent);
			$this->db->update('pegawai_talent_simpeg', $update_data2);
		...
		$n = 1;
		foreach ($pegawai as $p) {
			echo "[$n]" . $p->nama_lengkap . " = Box " . $p->posisi_box . "\n";
			$n++;
		}
	}

	public function reset_talent()
	{
		$ids = $this->ids_inject;
