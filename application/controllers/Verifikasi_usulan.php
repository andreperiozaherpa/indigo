...
class Verifikasi_usulan extends CI_Controller {
	public $user_id;
	...
	public function index()
	{
		if ($this->user_id)
		{
			$data['title']		= "Verifikasi Usulan - Admin ";
			$data['content']	= "verifikasi_usulan/index" ;
			$data['user_picture'] = $this->user_picture;
			$data['full_name']		= $this->full_name;
			$data['user_level']		= $this->user_level;
			$data['active_menu']		= 'verifikasi_usulan';

			$hal = 6;
			$page = isset($_GET["page"]) ? (int)$_GET["page"] : 1;
			$mulai = ($page>1) ? ($page * $hal) - $hal : 0;
			$total = count($this->usulan_pensiun_model->get_all(true));
			...
			$data['list'] = $this->usulan_pensiun_model->get_page($mulai,$hal,$nama,$skpd,true,$status);
			$data['skpd'] = $this->ref_skpd_model->get_all();
			$data['perihal'] = $this->perihal;
			$data['c_pending'] = count($this->usulan_pensiun_model->get_by_status('pending'));
			$data['c_terima'] = count($this->usulan_pensiun_model->get_by_status('terima'));
			$data['c_tolak'] = count($this->usulan_pensiun_model->get_by_status('tolak'));
			$data['c_total'] = count($this->usulan_pensiun_model->get_all(true));
	...
	public function view($id_usulan=0)
	{
		if ($this->user_id && !empty($id_usulan) && $id_usulan>0)
		{
			...
			$data['title']		= "Detail Usulan Pensiun - Admin ";
			$data['content']	= "verifikasi_usulan/view" ;
			$data['user_picture'] = $this->user_picture;
			$data['full_name']		= $this->full_name;
			$data['user_level']		= $this->user_level;
			$data['active_menu']		= 'verifikasi_usulan';
			if(!empty($_POST)){
				...
				if($upd['status_usulan']=="terima"){
					$data['message'] = 'Usulan berhasil diterima';
					$data['type'] = 'success';
				}else{
					$data['message'] = 'Usulan telah ditolak';
					$data['type'] = 'danger';
				}
			}
