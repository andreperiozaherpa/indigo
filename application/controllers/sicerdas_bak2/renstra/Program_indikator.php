...
class Program_indikator extends CI_Controller
{
        ...
        if($this->input->is_ajax_request()  )
        {
            if($_POST)
            {
                ...
                if( $this->form_validation->run() )
                {
                    ...
                    if($id_indikator_program_renstra)
                    {
                        $this->db->where("id_indikator_program_renstra",$id_indikator_program_renstra)->delete("sc_renstra_program_indikator_unit_kerja");
                        foreach($_POST['ids_unit_kerja'] as $key => $value)
                        {
                            ...
                            ->insert("sc_renstra_program_indikator_unit_kerja");
                        }
                    }
                }
                else{
    ...
    public function get_indikator_by_program()
    {
        ...
        if($this->input->is_ajax_request() && $this->input->post("id_program_renstra"))
        {
            ...
            foreach($dt->result() as $row)
            {
                $content .= '<option value="'.$row->id_indikator_program_renstra.'">'.$row->nama_indikator_program_renstra.'</option>';
            }           
        }
        $data['content'] = $content;
        echo json_encode($data);
