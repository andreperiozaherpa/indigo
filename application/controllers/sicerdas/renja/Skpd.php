...
class Skpd extends CI_Controller
{
	...
	public function get_skpd($rowno=1)
    {
        if($this->input->is_ajax_request())
        {
			...
			$param_total['str_where'] = "(sasaran.id_skpd in (".implode(",",$ids_skpd).") )";
			$rs_total = $this->Sub_kegiatan_model->getTotal($param_total);
			$total = array();
			foreach($rs_total->result() as $row)
			{
            ...
            {
				$jumlah = isset($total[$row->id_skpd]) ? $total[$row->id_skpd] : 0 ;
                $content .= '
					...
					<div class="white-box">
						...
						<div class="row b-b">
									<div class="col-md-12 col-sm-4 text-center b-r">
										<h3 class="box-title m-b-0">'.$jumlah.'</h3>
										Sub Kegiatan
									</div>
						...
						<div class="row">
							<div class="col-md-12">
								<br>
								<address>
									<a href="'.base_url().'/sicerdas/renja/subkegiatan?token='.md5("SKPD".$row->id_skpd).'">
