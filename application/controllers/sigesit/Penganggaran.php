...
class Penganggaran extends CI_Controller
{
	...
	public function detail($token)
	{
		...
		if($detail)
		{
			...
			$dt_anggaran = $this->db->where("id_kegiatan",$detail->id_kegiatan)->get("sigesit_anggaran")->row();
            if(!$dt_anggaran)
            {
                // insert anggaran
                ...
                $anggaran = array(
                    ...
                    'id_ref_program'   => $detail->id_ref_program,
                    'id_ref_kegiatan'   => $detail->id_ref_kegiatan,
                    'id_ref_sub_kegiatan'   => $detail->id_ref_sub_kegiatan,
                    'total_anggaran'   => ($total_pagu > 0) ? $total_pagu : $detail->rencana_anggaran,
                    'target_anggaran'   => $detail->target,
                    'satuan_anggaran'   => $detail->satuan,
                );
                $this->db->insert("sigesit_anggaran",$anggaran);
                $dt_anggaran = $this->db->where("id_kegiatan",$detail->id_kegiatan)->get("sigesit_anggaran")->row();
            }
            //echo "<pre>";print_r($dt_anggaran);die;
            $data['dt_anggaran'] = $dt_anggaran;
			if($data['dt_anggaran'])
			{
				$data['dt_kegiatan'] = $this->Kegiatan_model->get_kegiatan($data['dt_anggaran']->id_ref_program,false)->result();
				$data['dt_sub_kegiatan'] = $this->Kegiatan_model->get_sub_kegiatan($data['dt_anggaran']->id_ref_kegiatan,false)->result();
			}
			
			$dt_anggaran_detail = $this->db->where("id_kegiatan",$detail->id_kegiatan)->get("sigesit_anggaran_detail")->result();
            if(!$dt_anggaran_detail)
            {
                $dt_alokasi_anggaran = $this->db->where("id_kegiatan",$detail->id_kegiatan)->get("sigesit_alokasi_anggaran")->result();
                foreach($dt_alokasi_anggaran as $row)
                {
                    $anggaran_detail = array(
                    ...
                    $this->db->insert("sigesit_anggaran_detail",$anggaran_detail);
                }

                $dt_anggaran_detail = $this->db->where("id_kegiatan",$detail->id_kegiatan)->get("sigesit_anggaran_detail")->result();
            }
            $data['dt_anggaran_detail'] = $dt_anggaran_detail;
        ...
        if($this->input->is_ajax_request()  )
        {
            if($_POST)
            {
                ...
                if( $this->form_validation->run() )
                {
                    ...
                    {
                        $total_pagu = $this->Realisasi_model->get_total_pagu($detail->kode_sub_kegiatan, $detail->tahun);
                        $total_anggaran = str_replace(",","",$post_data['total_anggaran']);
                        $target_anggaran = str_replace(",","",$post_data['target_anggaran']);

                        $maximal_anggaran = ($total_pagu > 0) ? $total_pagu : $detail->rencana_anggaran;
    
                        if($total_anggaran > $maximal_anggaran)
                        {
                            $data['status'] = false;
                            $data['message'] = "Anggaran melebihi batas maksimal anggaran, yaitu Rp. ".number_format($maximal_anggaran);
                        }
                        else{
                            $dt = array(
                                ...
                                'satuan_anggaran'    => $post_data['satuan_anggaran'],
                            );
        
                            $cek = $this->db 
                            ->where("id_kegiatan",$post_data['id_kegiatan'])
                            ->get("sigesit_anggaran")
                            ->row();
        
                            if($cek){
                                $this->db->where("id_kegiatan",$post_data['id_kegiatan'])->update("sigesit_anggaran",$dt);
                            }
                            else{
                                $this->db->insert("sigesit_anggaran",$dt);
                            }
        
                            // update kegiatan
                            $this->db
                            ->set("anggaran",$total_anggaran)
                            ->where("id_kegiatan",$post_data['id_kegiatan'])
                            ->update("sigesit_kegiatan");
        
                            $data['message'] = "Anggaran berhasil disimpan";
                        }
                    }
                    else{
                        $data['status'] = false;
        ...
        if($this->input->is_ajax_request()  )
        {
            if($_POST)
            {
                ...
                if( $this->form_validation->run() )
                {
                    ...
                    {
                        $this->db->where("id_kegiatan",$post_data['id_kegiatan'])->delete("sigesit_anggaran_detail");
                        ...
                        {
                            $total_pagu = $this->Realisasi_model->get_total_pagu($detail->kode_sub_kegiatan, $detail->tahun);
                            $max_anggaran = ($total_pagu > 0 ) ? $total_pagu  : $detail->anggaran;
                            
                            if($total_anggaran != $max_anggaran)
                            {
                                $data['status'] = false;
                                $data['message'] = "Jumlah Anggaran tidak sama. Total anggaran harus Rp. ".number_format($max_anggaran);
                            }
                            else{
                                foreach($anggaran['nama_kegiatan'] as $key => $value)
                                {
                                    if($value){
                                        ...
                                        $this->db->insert("sigesit_anggaran_detail",$realisasi);
                                    }
                                }
                                $data['message'] = "Anggaran berhasil disimpan";
                            }
