...
class Lomba_inovasi extends CI_Controller 
{
	public $user_id;
	...
	public function __construct(){
		...
		// $this->load->model('lomba_inovasi_model', 'iim');
	...
	public function index()
	{
		if ($this->user_id)
		{
			...
			$data['content']	= "lomba_inovasi/index" ;
			$data['user_picture'] = $this->user_picture;
			$data['full_name']		= $this->full_name;
			$data['user_level']		= $this->user_level;
			$data['active_menu'] = "lomba_inovasi";
			...
			// 	unset($filter['filter']);
			// }
			// if ($this->user_level != 'Administrator' && !in_array('program', $user_privileges) ) {
			// 	$this->iim->id_user = $this->user_id;
			// }
			// $data['list'] = $this->iim->get_lomba_inovasi($filter);
	...
	public function add()
	{
		if ($this->user_id)
		{
			...
			$data['content']	= "lomba_inovasi/add" ;
			$data['user_picture'] = $this->user_picture;
			$data['full_name']		= $this->full_name;
			$data['user_level']		= $this->user_level;
			$data['active_menu'] = "lomba_inovasi";
			...
			// 		if ($value->name == 'id_user' || $value->name == 'created_at' || $value->name == 'updated_at' || $value->name == 'id_lomba_inovasi' || $value->name == 'id_skpd' 
			...
			// 				'max_size'      => 5000,
			// 				'overwrite'     => FALSE,
			// 				'upload_path' 	=> './data/lomba_inovasi/'
			// 			);
	...
	public function detail($id)
	{
		if ($this->user_id) {
			...
			$data['content']	= "lomba_inovasi/detail" ;
			$data['user_picture'] = $this->user_picture;
			$data['full_name']		= $this->full_name;
			$data['user_level']		= $this->user_level;
			$data['active_menu'] = "lomba_inovasi";
			...
			$data['column'] = $this->iim->get_column_name();
			$data['detail'] = $this->iim->get_lomba_inovasi_by_id($id);
	...
	public function delete($id)
	{
		if ($this->user_id) {
			...
			if (file_exists('./data/lomba_inovasi/'.$detail['jumlah_jenis_pelayanan_file'])) {
				unlink('./data/lomba_inovasi/'.$detail['jumlah_jenis_pelayanan_file']);
			}

			foreach ($data['column'] as $key => $value) {
				if (substr($value->name, -5) == '_file') {
					if (!empty($detail[$value->name.'_file'])) {
						if (file_exists('./data/lomba_inovasi/'.$detail[$value->name.'_file'])) {
							unlink('./data/lomba_inovasi/'.$detail[$value->name.'_file']);
						}
					}
				}
				if (substr($value->name, -5) == '_foto') {
					if (!empty($detail[$value->name.'_foto'])) {
						if (file_exists('./data/lomba_inovasi/'.$detail[$value->name.'_foto'])) {
							unlink('./data/lomba_inovasi/'.$detail[$value->name.'_foto']);
						}
					}
				}
			}
	...
	// 		$data['content']	= "lomba_inovasi/edit" ;
	// 		$data['user_picture'] = $this->user_picture;
	// 		$data['full_name']		= $this->full_name;
	// 		$data['user_level']		= $this->user_level;
	// 		$data['active_menu'] = "lomba_inovasi";
