...
class Kinerja_tambahan extends CI_Controller
{
    ...
    public function download($token)
    {
        ...
        $content = $this->Kinerja_tambahan_model->getContent($detail,true);
        
        $html = '
            <p style="text-align:center"><b>KINERJA TAMBAHAN</b><br>
        ...
        $pdf->writeHTML($html);
        ob_end_clean();
        $pdf->Output('Kinerja_Tambahan_'.$detail->nama_lengkap.'_tahun_'.$detail->tahun_desc.'.pdf', 'D');
    }
    ...
    public function get_data()
    {
        ...
        if($this->input->is_ajax_request() && $id_skp)
        {
            ...
            $content = $this->Kinerja_tambahan_model->getContent($detail);
